            ...
            $element.keydown(function (e) {
                ...
                if (e.keyCode === 13 || e.keyCode === 32) {
                    ...
                    if (/^[a-z0-9._-]+@[a-z0-9._-]+\.[a-z]{2,6}$/.test(getValue)){
                        $('.all-mail').append('<span class="email-ids">' + '<span class="single-email-id">' + getValue + '</span >'+ '<span class="cancel-email">x</span></span>');
                        $element.val('');
            ...
            if(settings.data){
                $.each(settings.data, function (x, y) {
                    if (/^[a-z0-9._-]+@[a-z0-9._-]+\.[a-z]{2,6}$/.test(y)){
                        $('.all-mail').append('<span class="email-ids">' + '<span class="single-email-id">' + y + '</span>' + '<span class="cancel-email">x</span></span>');
                        $element.val('');
